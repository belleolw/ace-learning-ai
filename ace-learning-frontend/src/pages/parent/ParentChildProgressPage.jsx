...
export default function ParentChildProgress() {
  ...
  const topicProgress = useMemo(() => {
    ...
    return topicMastery.map((item, index) => {
      ...
      const previousScore = Math.round(Number(item.previous_score || item.score || 0))
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className={`mt-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-medium ${improvementRate >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>
